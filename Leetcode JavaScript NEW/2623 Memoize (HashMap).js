...
function memoize(fn) {
    ...
    return function(...args) {
        ...
        //检查keyStr 是否已存在于缓存 meoize中。
        //如果不存在，则调用提供的函数，得到对应的输出值
        if(!map.has(keyStr)){
          const functionOutput = fn(...args); //调用提供的函数，得到输出值
          map.set(keyStr, functionOutput); //然后把fn和对应的输出值在重新存入hashmap中
        }
        //如果已经存在于缓存meoize中，表示生成过当前fn的输出，则立即返回关联值。
